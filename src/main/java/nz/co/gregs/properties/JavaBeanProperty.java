...
import nz.co.gregs.properties.exceptions.DBPebkacException;
...
public class JavaBeanProperty implements JavaProperty {
	...
	public Object get(Object target) {
		...
		try {
			...
			throw new PropertyException("Internal error reading property " + qualifiedName() + " on object of type " + class1 + " (this is probably a DBvolution bug): " + e.getLocalizedMessage(), e);
		} catch (IllegalAccessException e) {
			// caused by a Java security manager or an attempt to access a non-visible field
			// without first making it visible
			throw new PropertyException("Java security error reading property " + qualifiedName() + ": " + e.getLocalizedMessage(), e);
	...
	public void set(Object target, Object value) {
		...
		try {
			...
			// caused by a Java security manager or an attempt to access a non-visible field
			// without first making it visible
			throw new PropertyException("Java security error writing to property " + qualifiedName() + ": " + e.getLocalizedMessage(), e);
		} catch (InvocationTargetException e) {
			// any checked or runtime exception thrown by the setter method itself
			Throwable cause = (e.getCause() == null) ? e : e.getCause();
			String msg = (cause.getLocalizedMessage() == null) ? "" : ": " + cause.getLocalizedMessage();
		...
		for (Method method : methods) {
			if (!method.getDeclaringClass().isAssignableFrom(Annotation.class)) {
				try {
					...
					throw new PropertyException("Internal error reading annotation value " + name + " (this is probably a DBvolution bug): " + e.getLocalizedMessage(), e);
				} catch (IllegalAccessException e) {
					// caused by a Java security manager or an attempt to access a non-visible field
					// without first making it visible
					String name = "@" + annotation.annotationType().getSimpleName() + "." + method.getName();
					throw new PropertyException("Java security error reading annotation value " + name + ": " + e.getLocalizedMessage(), e);
				} catch (InvocationTargetException e) {
					Throwable cause = (e.getCause() == null) ? e : e.getCause();
					String msg = (cause.getLocalizedMessage() == null) ? "" : ": " + cause.getLocalizedMessage();
					String name = "@" + annotation.annotationType().getSimpleName() + "." + method.getName();
					throw new PropertyException("Internal error reading annotation value " + name + " (this is probably a DBvolution bug)" + msg, cause);
				}
			}
		}
		return values;
